...
export const timeForToday = (value: string, type: string) => {
  ...
  if (type === 'notification') {
    ...
    if (fullday.slice(0, 10) === value.slice(0, 10)) return value.slice(11, 16);
...
export const whoIsWriter = (
  ...
  if (isAuthor) return '글쓴이';
  else return `카뎃${commentsUserList.indexOf(authorId) + 1}`;
};
...
export const makeCommentUserList = (comments: CommentData[]) => {
  ...
  for (let cmt of comments) {
    if (!cmt.isAuthor) result.push(cmt.authorId);
    if (cmt.recomments !== null) {
      for (let recmt of cmt.recomments) {
        if (!recmt.isAuthor) result.push(recmt.authorId);
      }
    }
  }
